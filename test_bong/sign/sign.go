...
func GenerateKeyHandler(w http.ResponseWriter, r *http.Request) {
	...
	// config.yml 업데이트 (주석과 형식 유지)
	err = config.SaveConfig(privateKeyFile, publicKeyFile)
	if err != nil {
		log.Printf("config.yml 업데이트 실패: %v", err)
		http.Error(w, "config.yml 업데이트 실패", http.StatusInternalServerError)
		return
	}
	log.Println("config.yml 업데이트 - 성공")

	// 업데이트된 파일 경로 로그 출력
	log.Printf("개인키 파일 경로: %s", privateKeyFile)
	log.Printf("공개키 파일 경로: %s", publicKeyFile)
